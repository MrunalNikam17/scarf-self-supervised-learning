...
exactly like the control. This tests whether pre-trained representations are
more robust to label noise than randomly-initialized ones.

Usage:
    python scripts/run_label_noise.py
"""
...
NOISE_FRAC = 0.30  # matches the paper's 30% label-noise setting
...
def main():
    ...
    device = "cpu"
    out_dir = os.path.join(os.path.dirname(HERE), "results_label_noise")
    ...
    for name, fname in DATASETS.items():
        ...
        print(f"\n=== {name}: {X.shape[0]} rows x {X.shape[1]} raw features, "
              f"{y.nunique()} classes, {NOISE_FRAC:.0%} label noise ===")
        t_dataset = time.time()
        ...
        for trial in range(n_trials):
            ...
            y_train_noisy = corrupt_labels(
                splits.y_train, splits.n_classes, noise_frac=NOISE_FRAC, random_state=trial
            )
            ...
            trial_results = run_all_combinations(
                ...
                x_train_ft=splits.x_train,        # fine-tuning sees the FULL (noisy) training set
